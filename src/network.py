...
class Network(object):
    ...
    def feedforward(self, a):
        """Return the output of the network if 'a' in an input"""
    ...
    def SGD(self, training_data, epochs, mini_batch_size, eta, test_data=None):
        ...
        gradient descent.  The "training_data" is a list of tuples "(x, y)" representing the training inputs and the desired outputs.  The other non-optional parameters are self-explanatory.  If "test_data" is provided then the network will be evaluated against the test data after each epoch, and partial out.  This is useful for tracking progress, but slows things down substantially."""
        ...
        for j in range(epochs):
            ...
            if test_data:
                print("Epoch {0}: {1} / {2}".format(
                    j, self.evalute(test_data), n_test
                ))
            else:
